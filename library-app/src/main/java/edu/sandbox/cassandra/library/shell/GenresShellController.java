...
import edu.sandbox.cassandra.library.services.GenresService;
...
public class GenresShellController {
    ...
    public String create(@ShellOption("-n") String name) {
        edu.sandbox.cassandra.library.domain.Genre genre = genreArgumentMapper.map(name);
    ...
    public String update(@ShellOption("-i") String id, @ShellOption("-n") String name) {
        edu.sandbox.cassandra.library.domain.Genre genre = genreArgumentMapper.map(id, name);
        return genreToStringResponseMapper.map(genresService.save(genre));
    }
